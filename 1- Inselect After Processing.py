'''
RUN ON PYTHON 3.6 
Inselect Source Code obtained from https://github.com/NaturalHistoryMuseum/inselect/blob/master/inselect/lib/segment.py
...
Some functions were not used, code was re-written to remove redundant parts
Added functions:
whitened_borders to remove tray border
determine_insect_size to classify the insects and to apply further processing techniques
get_overlap to remove overlapped rectangles
merge_rectangles to merge nearby rectangles
other functions such as filtering and saving to .xml
''' 
...
def whitened_borders(image):
    ...
    for i in range(len(corners)):
        # If the x coordinate is more than upper limit and less than lower limit ##
...
def determine_insect_size(rects, arealist, display):
    ...
    if (prediction_final >= 3):
        ...
        rects_bi, numofrects_bi = draw_rectangles_biginsects(bigrects_analysed, display)
        ...
        rects_si, numofrects_si = draw_rectangles_smallinsects(removedoverlappedrects_small, display)
...
def save_coordinates_to_xml(filename,numofrects,rectscoords):
    ...
    xmlfile = re.sub(".jpg","Inselect_after_processing.xml",filename)
...
if __name__ == "__main__":
    ...
    cv2.imshow('Ovelrapped',outputimager)
